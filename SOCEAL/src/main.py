...
# Ensure project root is on the path -- handle PyInstaller frozen mode
if getattr(sys, 'frozen', False) and hasattr(sys, '_MEIPASS'):
    PROJECT_ROOT = Path(sys._MEIPASS)
    _EXE_DIR = Path(sys.executable).parent
else:
    PROJECT_ROOT = Path(__file__).resolve().parent.parent
    _EXE_DIR = PROJECT_ROOT
...
def ensure_admin():
    """Check for admin privileges and offer to relaunch elevated."""
    if os.name != 'nt':
        return
    ...
    logger.warning("Not running as administrator -- some features will be limited")
    print("  NOTE: Not running as Administrator -- some features will be limited.")
    print("  TIP:  Right-click RUNME.bat -> 'Run as administrator' for full access.")
    print()
...
def main():
    ...
    # -- Load config --
    # When frozen, look for config next to the exe (user-editable)
    if getattr(sys, 'frozen', False):
        default_config = _EXE_DIR / 'config' / 'config.yaml'
    else:
        ...
        if not args.no_ui:
            if args.webview:
                ...
                threading.Thread(
                    ...
                    kwargs={
                        'safe_mode': safe_mode,
                        'on_close': lambda: signal_handler(None, None),
                    },
                    daemon=True,
                ).start()
            else:
